...
alg::w_sparseMat Kw(NOD);//write_matrix Kw(NOD, NOD);
Kw.push_back(0,0,1.0);//Kw(0,     0    )=+1.0;  Kw(0,     1    )=-1.0;
Kw.push_back(0,1,-1.0);
Kw.push_back(NOD-1,NOD-2,-1.0); Kw.push_back(NOD-1,NOD-1,1.0);//Kw(NOD-1, NOD-2)=-1.0;  Kw(NOD-1, NOD-1)=+1.0;
...
/*
    ...
    if (r>0.7){ld.push_back(n);Vd[n]=1.0;}
*/
}
ld.push_back(0);
Vd[0]=0.0;
...
alg::r_sparseMat Kr(Kw);//read_matrix  Kr(NOD, NOD);    gmm::copy(Kw, Kr);


std::vector<double> Lr(NOD,0.0);//read_vector  Lr(NOD);         
Lr.assign(Lw.begin(),Lw.end());//gmm::copy(Lw, Lr);

alg::mult(Kr,Xw,Lw);//gmm::mult(Kr, Xw, Lw);
