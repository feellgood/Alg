...
template<bool T>
void maskedLinComb(std::vector<size_t> const& v_idx, std::vector<bool> const &b, alg::r_sparseMat const& A,std::vector<double> const& X,std::vector<double> const&B,std::vector<double> &Y,const std::function<double(double,double)> Op)
{
